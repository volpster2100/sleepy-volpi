...
function useInBedIncrements(): [boolean, ITimeIncrementOption[]] {
    ...
    // mimic network time
        setTimeout(() => {
            const calculatedOptions: ITimeIncrementOption[] = [];
            ...
            for (let i = 0; i < increments.length; i++) {
                ...
                const hourLabel =
          hours === 1 ? t('page.common.hour') : t('page.common.hours');

                calculatedOptions.push({
                    label: `${hours} ${hourLabel}, ${minutes} ${t(
                        'page.common.minutes'
                    )}`,
                    value: increment,
                });
...
export default useInBedIncrements;
